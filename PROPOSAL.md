...
8. **代码生成实验与 descriptor bridge**：生成 MoonBit struct、enum、descriptor 和 helper；支持 FileDescriptorSet 导入、重建 proto 子集、old/new descriptor 兼容性检查、registry 版本索引、发布前兼容性 gate 与 JSON release-policy gate，并通过 compile check 保证生成结果可构建。

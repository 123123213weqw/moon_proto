...
| `moon_package_check.png` | `moon_package_check.txt` | `moon package` |
...
The README front page intentionally uses four non-overlapping screenshots: release gate overview, multi-target tests, JSON roundtrip functionality and package publishing gate. The `schema_doctor_bad_order` command is still kept as supplemental evidence and is intentionally expected to exit with code `1`, because it proves the doctor rejects a deliberately broken AI-generated schema.
